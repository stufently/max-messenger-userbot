...
check_token() {
    local token status
    # Токен создаётся при первом обращении и лежит в каталоге данных. Отказ
    # означал бы, что каталог недоступен для записи, — то есть ровно ту
    # поломку, из-за которой затевалась вся правка каталога по умолчанию.
    #
    # Код возврата сохраняется отдельно, а не глушится `|| true`: иначе проверка
    # принимала бы за токен любой непустой вывод — например, текст объяснённого
    # отказа. Заодно сверяется форма: `secrets.token_urlsafe` даёт только буквы,
    # цифры, `-` и `_`, поэтому сообщение с пробелами или переносами сюда не
    # пролезет. Длина не закреплена — она дело `config.py`, а не этой проверки.
    # Само значение в сообщения не попадает ни при каком исходе: скрипт гоняют в
    # CI, а туда токен выводить нельзя — даже когда кажется, что в переменной
    # текст ошибки. Для разбора хватает кода возврата и длины.
    status=0
    token="$(docker exec "$name" maxub token)" || status=$?
    if [ "$status" != "0" ]; then
        echo "команда токена завершилась с кодом $status (вывод — выше, в stderr)" >&2
        return 1
    fi
    if ! printf '%s' "$token" | grep -Eq '^[A-Za-z0-9_-]{20,}$'; then
        echo "ответ не похож на токен: длина ${#token}, форма не та" >&2
        return 1
    fi
