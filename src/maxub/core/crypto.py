...
зашифрованными. Ключ хранится **вне** базы: в переменной окружения, в хранилище
ОС или в отдельном файле с правами 0600. Кража одного файла БД без ключа
бесполезна.

Здесь только сама операция шифрования. Откуда берётся ключ и почему источники
идут именно в таком порядке — в [Settings.resolve_secret_key][
maxub.config.Settings.resolve_secret_key] и [keystore][maxub.core.keystore].
"""
