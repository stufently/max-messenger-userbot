...
MIGRATIONS: tuple[Migration, ...] = (
    ...
    Migration(
        version=3,
        summary="отказ от записи очереди без отправки",
        statements=(
            # Причина отказа хранится отдельно от `error`: там записано, почему
            # отправка не удалась, и подменять это решением человека значит
            # потерять единственное объяснение отказа при позднем разборе.
            "ALTER TABLE outbox ADD COLUMN discard_reason TEXT",
            # Момент решения — как `sent_at` у отправленных: терминальное
            # состояние обязано само отвечать, когда оно наступило, иначе по
            # записи, найденной через полгода, этого не восстановить.
